...
console.log('yr   league  ->FA   %    | stars  ->FA  re-signed  moved  unsigned | mean deal  star deal');
for (let s = 0; s < SEASONS; s++) {
  ...
  let expired = 0, starExpired = 0, starResigned = 0, starMoved = 0, starUnsigned = 0;
  rosteredIds.forEach(function (id) {
    if (before[id].years > 1) return;   // deal still running, never hit the market
  ...
  console.log(
    ...
    String(rosteredIds.length).padStart(5) + String(expired).padStart(6) +
    (100 * expired / Math.max(1, rosteredIds.length)).toFixed(0).padStart(5) + '%   | ' +
...
console.log('  league reaching free agency:  ' + totals.expired + '/' + totals.rostered +
  '  (' + (100 * totals.expired / totals.rostered).toFixed(1) + '% of all rostered players, every single year)');
